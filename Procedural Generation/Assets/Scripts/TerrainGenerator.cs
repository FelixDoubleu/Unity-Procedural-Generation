...
public class TerrainGenerator : MonoBehaviour {
    ...
    private bool ErosionStarted = false;
    private string numIterationsString;
    private int numErosions = 1024;
    public void StartErosion(string numIterationsString_)
    {
        numIterationsString = numIterationsString_;
        if(showProgress)
        {
            numErosions = 1024;
            ErosionStarted = true;
        ...
        {
            numErosions = numErosionIterations;
    ...
    private void DoErosion()
    {
        ...
        if (printTimers)
        {
            Debug.Log($"{mapSize}x{mapSize} heightmap generated in {heightMapTimer}ms");
    ...
    private void Update()
    {
        if (ErosionStarted)
        {
            DoErosion();
            numErosions += 100;
            if (numErosions > numErosionIterations)
            {
                ErosionStarted = false;
            }
        }
    ...
    public void Erode () {
        int numThreads = numErosions / 1024;
        ...
        int[] randomIndices = new int[numErosions];
